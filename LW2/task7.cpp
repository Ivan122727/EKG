...
static void RenderSceneCB() {
	...
	glUniformMatrix4fv(gWorldLocation7, 1, GL_TRUE, &World.m[0][0]);
	...
	glutPostRedisplay(); // или glutIdleFunc(RenderSceneCB)
...
static void InitializeGlutCallbacks() {
	...
	//glutIdleFunc(RenderSceneCB); // или постредисплей вызывать
}
...
static void CompileShaders() {
	...
	gWorldLocation7 = glGetUniformLocation(ShaderProgram, "gWorld");
	assert(gWorldLocation7 != 0xFFFFFFFF);
}

void invoke7(int argc, char** argv) {
	glutInit(&argc, argv);
